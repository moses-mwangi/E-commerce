"use client";

import { Button } from "@/components/ui/button";
import Image from "next/image";
import React, { useEffect, useState } from "react";
import { SlArrowLeft, SlArrowRight } from "react-icons/sl";

interface Image {
  images: string[] | undefined;
  selectedImage: number;
  setSelectedImage: React.Dispatch<React.SetStateAction<number>>;
}

export default function ImageScrol({
  images,
  selectedImage,
  setSelectedImage,
}: Image) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [imagesPerPage, setImagesPerPage] = useState(5);

  useEffect(() => {
    function updateImagesPerPage() {
      const width = window.innerWidth;
      if (width < 440) {
        setImagesPerPage(2);
      } else if (width < 540) {
        setImagesPerPage(3);
      } else if (width < 650) {
        setImagesPerPage(4);
      } else {
        setImagesPerPage(5); // Default for desktop
      }
    }

    updateImagesPerPage();
    window.addEventListener("resize", updateImagesPerPage);
    return () => window.removeEventListener("resize", updateImagesPerPage);
  }, []);

  useEffect(() => {
    if (window.innerWidth < 640) {
      setCurrentIndex(0);
      setImagesPerPage(images ? images?.length : 5);
    }
  }, [images?.length, images]);

  function handlePrev() {
    if (images && images.length > 1 && currentIndex > 0) {
      setCurrentIndex((el) => el - 1);
      setImagesPerPage((el) => el - 1);
    }
  }

  function handleNext() {
    if (images && imagesPerPage + 1 < images?.length) {
      setCurrentIndex((el) => el + 1);
      setImagesPerPage((el) => el + 1);
    }
  }

  if (!images) return null;

  return (
    <div className=" flex items-center justify-center gap-2">
      <Button
        className="disabled:cursor-not-allowed w-9 h-9  rounded-full hover:text-black hover:bg-slate-50 bg-slate-100 font-semibold"
        onClick={() => {
          handlePrev();
        }}
        disabled={currentIndex === 0}
      >
        <SlArrowLeft className=" text-black font-bold" />
      </Button>
      <div className="">
        <div className="overflow-x-scroll sm:overflow-x-hidden hide-scrollbar flex gap-3">
          {images?.slice(currentIndex, imagesPerPage).map((image, idx) => (
            <Image
              onClick={() => {
                setSelectedImage(idx);
              }}
              className={`${
                selectedImage === idx ? "ring-2 ring-orange-500" : ""
              } rounded-lg cursor-pointer w-24 h-20`}
              key={idx}
              src={image}
              alt="nnnn"
              width={imagesPerPage === 1 ? 200 : 150}
              height={imagesPerPage === 1 ? 100 : 100}
            />
          ))}
        </div>
      </div>
      <Button
        className="w-9 h-9  rounded-full hover:text-black hover:bg-slate-50 bg-slate-100 font-semibold"
        onClick={() => {
          handleNext();
        }}
        disabled={imagesPerPage + 1 === images?.length}
      >
        <SlArrowRight className=" text-black font-bold" />
      </Button>
    </div>
  );
}

// "use client";

// import { Button } from "@/components/ui/button";
// import Image from "next/image";
// import React, { useEffect, useState, useCallback } from "react";
// import { SlArrowLeft, SlArrowRight } from "react-icons/sl";

// interface ImageScrollerProps {
//   images: string[] | undefined;
//   selectedImage: number;
//   setSelectedImage: React.Dispatch<React.SetStateAction<number>>;
// }

// export default function ImageScroller({
//   images,
//   selectedImage,
//   setSelectedImage,
// }: ImageScrollerProps) {
//   const [currentIndex, setCurrentIndex] = useState(0);
//   const [visibleImagesCount, setVisibleImagesCount] = useState(5);
//   const [itemWidth, setItemWidth] = useState(96); // Default width for mobile

//   const updateResponsiveSettings = useCallback(() => {
//     const width = window.innerWidth;

//     if (width < 440) {
//       setVisibleImagesCount(2);
//       setItemWidth(100);
//       // setItemWidth(70);
//     } else if (width < 540) {
//       setVisibleImagesCount(3);
//       setItemWidth(90);
//     } else if (width < 650) {
//       setVisibleImagesCount(4);
//       setItemWidth(85);
//     } else if (width < 768) {
//       setVisibleImagesCount(4);
//       setItemWidth(80);
//     } else {
//       setVisibleImagesCount(5);
//       setItemWidth(80);
//       // setItemWidth(96);
//     }
//   }, []);

//   useEffect(() => {
//     updateResponsiveSettings();
//     window.addEventListener("resize", updateResponsiveSettings);
//     return () => window.removeEventListener("resize", updateResponsiveSettings);
//   }, [updateResponsiveSettings]);

//   const handlePrev = useCallback(() => {
//     if (images && currentIndex > 0) {
//       setCurrentIndex((prev) => Math.max(0, prev - 1));
//     }
//   }, [currentIndex, images]);

//   const handleNext = useCallback(() => {
//     if (images && currentIndex + visibleImagesCount < images.length) {
//       setCurrentIndex((prev) => prev + 1);
//     }
//   }, [currentIndex, images, visibleImagesCount]);

//   const handleImageClick = useCallback(
//     (idx: number) => {
//       setSelectedImage(currentIndex + idx);
//     },
//     [currentIndex, setSelectedImage]
//   );

//   if (!images || images.length === 0) return null;

//   const showNavigation = images.length > visibleImagesCount;
//   const canGoPrev = currentIndex > 0;
//   const canGoNext = currentIndex + visibleImagesCount < images.length;

//   return (
//     <div className="flex items-center justify-center gap-2 w-full">
//       {showNavigation && (
//         <Button
//           variant="ghost"
//           size="icon"
//           className={`rounded-full hover:bg-gray-100 transition-colors ${
//             !canGoPrev ? "invisible" : ""
//           }`}
//           onClick={handlePrev}
//           disabled={!canGoPrev}
//           aria-label="Previous images"
//         >
//           <SlArrowLeft className="text-gray-700 text-lg" />
//         </Button>
//       )}

//       <div className="relative flex-1 overflow-hidden">
//         <div
//           className="flex transition-transform duration-300 ease-in-out gap-3 p-1"
//           style={{
//             transform: `translateX(-${currentIndex * (itemWidth + 12)}px)`,
//             width: `${images.length * (itemWidth + 12)}px`,
//           }}
//         >
//           {images.map((image, idx) => (
//             <div
//               key={idx}
//               className={`flex-shrink-0 rounded-lg cursor-pointer overflow-hidden ${
//                 selectedImage === idx ? "ring-2 ring-orange-500" : ""
//               }`}
//               style={{
//                 width: `${itemWidth}px`,
//                 height: `${itemWidth * 0.8}px`,
//               }}
//               onClick={() => handleImageClick(idx)}
//             >
//               <Image
//                 className="object-cover w-full h-full"
//                 src={image}
//                 alt={`Product image ${idx + 1}`}
//                 width={itemWidth}
//                 height={itemWidth * 0.8}
//                 priority={idx < visibleImagesCount}
//               />
//             </div>
//           ))}
//         </div>
//       </div>

//       {showNavigation && (
//         <Button
//           variant="ghost"
//           size="icon"
//           className={`rounded-full hover:bg-gray-100 transition-colors ${
//             !canGoNext ? "invisible" : ""
//           }`}
//           onClick={handleNext}
//           disabled={!canGoNext}
//           aria-label="Next images"
//         >
//           <SlArrowRight className="text-gray-700 text-lg" />
//         </Button>
//       )}
//     </div>
//   );
// }
