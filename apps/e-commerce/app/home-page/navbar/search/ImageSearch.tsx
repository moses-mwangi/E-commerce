    ...
    <>
      {!showModal && (
        <div className={`relative`}>
          ...
          {showPop === true && (
            <div
              className={`absolute top-12 -right-36 w-96 mx-auto p-5 border rounded-lg shadow-lg bg-white`}
            >
      ...
      {showModal === true && (
        <div
          className={`fixed inset-0 h-screen w-screen bg-black/20 backdrop-blur-sm z-50 flex items-center justify-center"`}
        >
          {showModal === false && <div>{icon}</div>}

          <Card
            className={` w-[400px] mx-auto px-8 py-6 border rounded-lg shadow-lg bg-white`}
          >
            <div className="flex justify-between">
              <h2 className="text-[17px] font-semibold text-center mb-4">
                Find Products with Image Search
